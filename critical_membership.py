...
chroma_db = Chroma(persist_directory="C:/Users/Devayani K/rag/db1", embedding_function=OpenAIEmbeddings(),
...
#query = "what do u know about wareconn"
...
def extract_text(links):
    ...
    for link in links:
        print('LINK: ',link,' ###########################################')
        querystring = {"token":"0aabee9fa0819a5a979c6cbc7a44201f","url":link,"timeout":"15000","fields":"text,images"}

        headers = {
          "x-rapidapi-key": "8dfd16f059msh44894bf420076a1p19605ejsnd26d064b2ce3",
          "x-rapidapi-host": "diffbot-diffbot.p.rapidapi.com"
        }

        response = requests.get(url, headers=headers, params=querystring)
        try:
            text.append(Document(page_content=response.json()['text']))
...
def web_search(state):
    ...
    return {"documents": texts, "question": question}

def grade_html_content(state):
    """
    Determines whether the retrieved documents are relevant to the question
    by using an LLM Grader.
    If any document are not relevant to question or documents are empty - Web Search needs to be done
    ...
    Returns:
        state (dict): Updates documents key with only filtered relevant documents
    """
    print("---CHECK HTML DOCUMENT RELEVANCE TO QUESTION---")
    from langchain.docstore.document import Document
    question = state["question"]
    documents = state["documents"]
    # Score each doc
...
SYS_PROMPT = """You are an expert grader assessing relevance of a retrieved document to a user question.
                Follow these instructions for grading:
                  - If the document has some semantic meaning related to the question, grade it as relevant.
...
tv_search = TavilySearchResults(max_results=5, search_depth='advanced',include_raw_content=True)
...
def agent():
    ...
    agentic_rag.add_node("grade_html_content", grade_html_content)  # grade documents
    ...
    agentic_rag.add_edge("web_search", "grade_html_content")
    agentic_rag.add_edge("grade_html_content", "generate_answer")
    agentic_rag.add_edge("generate_answer", "final_print")
    #agentic_rag.add_conditional_edges(
    #    "final_check",
    #    need_gemini,
